...
from aof2_gui import HandClass, Kind, find_2p_strategy_file
...
print(
    f"loaded: iters={strat.iterations:,} expl={strat.exploitability_bb:.3e} BB  SB EV={strat.sb_ev:+.4f} BB"
)
...
push_pct = (
    sum(
        (6 if h.kind is Kind.PAIR else (4 if h.kind is Kind.SUITED else 12))
        * (1 if strat.sb_push[h.index()] > 0.5 else 0)
        for r in range(13)
        for rh in range(13)
        for rl in range(13)
        if (rh == rl and r == rh) or (rh > rl and 0 <= rl < rh < 13)
        for h in [
            HandClass(rh, rh, Kind.PAIR)
            if rh == rl and r == rh
            else HandClass(rh, rl, Kind.SUITED if r == 0 else Kind.OFFSUIT)
        ]
...
print("SB push as fractions of all combos:")
push_combos = 0
...
def show(rh: int, rl: int, suited: bool, name: str | None = None):
    ...
    print(
        f"  {label:5}  SB push={strat.sb_push[h.index()] * 100:6.2f}%   BB call={strat.bb_call[h.index()] * 100:6.2f}%"
    )
